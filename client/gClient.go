...
func AskingDateTime(ctx context.Context, m protoapi.RandomClient) (*protoapi.DateTime, error) {
	request := &protoapi.RequestDateTime{
		Value: "Please send me date and time!",
	}
...
func main() {
	...
	conn, err := grpc.Dial(port, grpc.WithTransportCredentials(insecure.NewCredentials())) // Dial is used to connect grpc server
	if err != nil {
